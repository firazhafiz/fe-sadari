...
export default function Start() {
  const { setShowExitModal, formAnswer, saveAnswer, getAnswer, submitAnswers } =
    useAnswerContext();
  ...
  useEffect(() => {
    ...
    if (currentAnswer) {
      const answerIndex = soalHipertensi[currentSoal].pilihan.findIndex(
        (p) => p.text === currentAnswer
      );
  ...
  const selectAnswer = (index: number) => {
    saveAnswer(
      soalHipertensi[currentSoal].id,
      soalHipertensi[currentSoal].pilihan[index].text,
      soalHipertensi[currentSoal].pilihan[index].score
    );
  ...
  if (!showTest) {
    return (
      <div className="min-h-screen flex justify-center items-start w-full bg-gradient-to-br from-white to-gray-200 px-4 sm:px-6 md:px-12 lg:px-20 py-6">
  ...
  return (
    <div className="relative min-h-screen flex justify-center items-start w-full bg-gradient-to-br from-white to-gray-200 px-4 sm:px-6 md:px-12 lg:px-20 py-6 pb-28">
      <div className="w-full max-w-3xl  p-4 sm:p-6">
        ...
        <div className="flex flex-col items-start mt-8">
          <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
            <div
              className="bg-[#81BC66] h-2 transition-all duration-300 ease-in-out"
          ...
          {soalHipertensi[currentSoal].pilihan.map((item, idx) => (
            <div
              ...
              className={`w-full rounded-xl py-4 sm:py-6 px-4 sm:px-6 cursor-pointer shadow-md transition-all ease-in-out duration-200 font-medium text-start border-2 ${
                selectedAnswer === idx
                  ? "bg-[#81BC66] text-gray-100 md:text-sm border-[#81BC66] shadow-lg scale-[1.02] "
                  : "bg-gray-200 hover:bg-slate-300 md:text-sm text-xs  text-slate-800 border-transparent hover:border-gray-300"
              }`}
            >
      ...
      {/* Button Navigasi - fixed at bottom center */}
      <div className="absolute md:bottom-25 bottom-30 left-1/2 -translate-x-1/2 z-40 flex justify-center gap-8 sm:gap-12">
        <button
          onClick={handlePrev}
        ...
        >
          <Image
            ...
            width={30}
            height={30}
            className="h-6 sm:h-8 w-auto transform rotate-180"
          />
        </button>
      ...
      <div className="absolute  bottom-4 right-4 mt-6">
        <button
          onClick={handleExit}
          className="bg-[#FF5656] flex gap-2 justify-center items-center text-white text-sm sm:text-base cursor-pointer rounded-md px-4 sm:px-4 py-2 sm:py-2 hover:-translate-y-0.5 transition-all hover:shadow-md"
        >
          <IoExit />
          Keluar
        </button>
